<x-modal name="add-supplier" :show="$errors->storeSupplier->any()">
    <div class="bg-white px-6 pt-6 pb-6 relative">
        <div class="absolute top-4 right-4">
            <x-buttons.close-icon @click="$dispatch('close-modal', 'add-supplier')" />
        </div>

        <div class="mb-6">
            <h3 class="text-xl font-bold text-gray-900">Add Supplier</h3>
            <p class="text-sm text-gray-500 mt-1">Define a new supplier profile.</p>
        </div>

        @if($errors->storeSupplier->any())
            <div class="mb-4 bg-red-50 border border-red-200 text-red-600 px-4 py-3 rounded-lg text-sm">
                <ul class="list-disc pl-5">
                    @foreach($errors->storeSupplier->all() as $error)
                        <li>{{ $error }}</li>
                    @endforeach
                </ul>
            </div>
        ...
        <form action="{{ route('suppliers.store') }}" method="POST" novalidate>
            ...
            <div class="flex items-center justify-center space-x-3 pt-4 border-t border-gray-100">
                <x-buttons.cancel @click="$dispatch('close-modal', 'add-supplier')" class="w-1/2">Cancel</x-buttons.cancel>
