<x-modal name="delete-expense">
    <div class="bg-white px-6 pt-5 pb-6 relative text-center">
        <div class="absolute top-4 right-4">
            <x-buttons.close-icon @click="$dispatch('close-modal', 'delete-expense')" />
        </div>
        ...
        <p class="text-sm text-gray-500 mb-6">Are you sure you want to delete this expense of <span class="font-semibold text-gray-800">₹<span x-text="form.amount"></span></span>? This action cannot be undone.</p>
        
        <form :action="deleteAction" method="POST">
            @csrf
            @method('DELETE')
            <div class="flex items-center justify-center space-x-3">
                <x-buttons.cancel @click="$dispatch('close-modal', 'delete-expense')" class="w-1/2">Cancel</x-buttons.cancel>
