<x-modal name="edit-invoice" :show="$errors->updateInvoice->any()" maxWidth="5xl">
    <div class="bg-white px-6 pt-6 pb-6 relative">
        ...
        <div class="mb-6">
            <h3 class="text-xl font-bold text-gray-900">Edit Sales Invoice</h3>
            <p class="text-sm text-gray-500 mt-1">Update details for the selected sales invoice.</p>
        </div>
            ...
            <div class="bg-gray-50 rounded-xl border border-gray-200 p-4">
                <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                    ...
                    <div><label class="block text-sm font-medium text-gray-700 mb-1">Customer <span class="text-red-500">*</span></label>
                        <select name="customer_id" x-model="updateForm.customer_id" required class="w-full rounded-lg border-gray-300 shadow-sm focus:ring-emerald-500 focus:border-emerald-500">
                            <option value="">Select Customer</option>
                            @foreach($customers as $c)<option value="{{ $c->id }}">{{ $c->name }}</option>@endforeach
                        </select>
                    </div>
                    <div><label class="block text-sm font-medium text-gray-700 mb-1">Status</label>
                        <select name="status" x-model="updateForm.status" required class="w-full rounded-lg border-gray-300 shadow-sm focus:ring-emerald-500 focus:border-emerald-500">
                    ...
                    <table class="min-w-full divide-y divide-gray-200">
                        ...
                        <tbody class="divide-y divide-gray-100 bg-white">
                            <template x-for="(item, index) in updateForm.items" :key="index">
                                <tr>
                                    <td class="p-2">
                                        <select :name="'items['+index+'][product_id]'" x-model="item.product_id" @change="onUpdateProductSelect(index)" required class="w-full text-sm rounded-md border-gray-300 py-1.5 shadow-sm focus:ring-emerald-500 focus:border-emerald-500">
                                    ...
                                    <td class="p-2"><input type="number" :name="'items['+index+'][quantity]'" x-model.number="item.quantity" @input="calculateUpdateTotals()" min="1" required class="w-full text-sm rounded-md border-gray-300 py-1.5 shadow-sm focus:ring-emerald-500 focus:border-emerald-500"></td>
                                    <td class="p-2"><input type="number" step="0.01" :name="'items['+index+'][unit_price]'" x-model.number="item.unit_price" @input="calculateUpdateTotals()" required class="w-full text-sm rounded-md border-gray-300 py-1.5 shadow-sm focus:ring-emerald-500 focus:border-emerald-500"></td>
                                    <td class="p-2"><input type="number" step="0.01" :name="'items['+index+'][tax_amount]'" x-model.number="item.tax_amount" @input="calculateUpdateTotals()" class="w-full text-sm rounded-md border-gray-300 py-1.5 shadow-sm focus:ring-emerald-500 focus:border-emerald-500"></td>
