...
@section('content')
<div x-data="purchaseInvoiceManager()">
    <form method="GET" action="{{ route('purchase-invoices.index') }}">
        <x-filters.bar>
            <x-filters.search placeholder="Search invoice number, supplier..." />
            ...
            <x-slot:actions>
                <button type="button" @click="$dispatch('open-store')" class="inline-flex items-center justify-center w-10 h-10 bg-emerald-600 text-white rounded-lg hover:bg-emerald-700 transition shadow-sm" title="New Purchase Invoice">
            ...
            <tbody class="divide-y divide-gray-100">
                @forelse($invoices as $invoice)
                    <tr class="hover:bg-gray-50 transition">
                        <td class="px-6 py-4 text-sm font-medium text-emerald-600"><a href="{{ route('purchase-invoices.show', $invoice) }}">{{ $invoice->invoice_number }}</a></td>
                        <td class="px-6 py-4 text-sm text-gray-700">{{ $invoice->supplier->name ?? 'N/A' }}</td>
                        ...
                        <td class="px-6 py-4 text-sm font-semibold text-gray-800 text-right">₹{{ number_format($invoice->total_amount, 2) }}</td>
                        <td class="px-6 py-4"><span class="px-2 py-0.5 text-xs font-medium rounded-full {{ $invoice->status?->color() === 'green' ? 'bg-green-100 text-green-700' : ($invoice->status?->color() === 'red' ? 'bg-red-100 text-red-700' : 'bg-yellow-100 text-yellow-700') }}">{{ $invoice->status?->label() }}</span></td>
                        <td class="p-4">
                            <div class="flex justify-end items-center gap-3">
                                <x-actions.view @click="viewInvoice({{ $invoice->load('purchaseInvoiceItems', 'supplier')->toJson() }})" />
                                <x-actions.edit @click="editInvoice({{ $invoice->load('purchaseInvoiceItems')->toJson() }})" />
                                <form action="{{ route('purchase-invoices.destroy', $invoice) }}" method="POST" class="inline" onsubmit="return confirm('Delete?')">
                                    @csrf @method('DELETE')
                ...
                @empty
                    <tr><td colspan="7" class="px-6 py-12 text-center text-gray-400">No purchase invoices found.</td></tr>
                @endforelse
            </tbody>
        </table>
    ...
    @include('purchase-invoices.show')
    @include('purchase-invoices.store')
    @include('purchase-invoices.update')
</div>
...
<script>
    ...
    function purchaseInvoiceManager() {
        return {
            ...
            // Update methods
            editInvoice(invoice) {
                this.updateAction = `/purchase-invoices/${invoice.id}`;
                this.updateForm.id = invoice.id;
                this.updateForm.invoice_number = invoice.invoice_number;
                this.updateForm.supplier_id = invoice.supplier_id;
                ...
                this.updateForm.items = invoice.purchase_invoice_items.map(item => ({
            ...
            viewInvoice(invoice) {
                ...
                this.viewForm.supplier_name = invoice.supplier ? invoice.supplier.name : 'N/A';
                this.viewForm.supplier_email = invoice.supplier ? invoice.supplier.email : '';
                this.viewForm.supplier_phone = invoice.supplier ? invoice.supplier.phone : '';
                ...
                this.viewForm.tax_amount = invoice.tax_amount || 0;
                this.viewForm.total_amount = invoice.total_amount || 0;
                
                this.viewForm.items = invoice.purchase_invoice_items.map(item => {
                    const product = Object.entries(productPrices).find(([id]) => id == item.product_id);
                    // product mapping is basic here since we don't load the product relation in view, but we can do our best with description
                    return {
            ...
            payInvoice(invoice, type) {
                ...
                this.paymentForm.amount = invoice.total_amount - (invoice.paid_amount || 0);
                if (this.paymentForm.amount <= 0) this.paymentForm.amount = invoice.total_amount;
